...
%macro unpack(unzip /* Full path pointing to where to create the EST142 data */
...
%let url=%str(http://dl.liveweb.sas.com/eldata/est142_oda.zip);
...
%let setup=%superq(unzip)/createdata.sas;
...
%unpack(%superq(path),est142.zip,est142)

/* added to set &path for elearning */
%let path=/home/&sysuserid/EST142/data;
